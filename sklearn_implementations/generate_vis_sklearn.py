...
# Load the k-fold data (updated column names)
df = pd.read_csv('results/sweep/kfold_sweep_results.csv')
...
def create_plots(data_subset, plot_type, save_dir, title_suffix=""):
    """Create the four plot types for a given data subset with k-fold error bars"""
    ...
    for experiment in data_subset['experiment'].unique():
        ...
        plt.xlabel('Number Precision (size)', fontsize=12)
        plt.ylabel('Linear R² (Mean ± Std)', fontsize=12)
        plt.ylim(linear_r2_lower - 0.05, linear_r2_upper + 0.05)
        plt.legend(loc='lower center', bbox_to_anchor=(0.5, -0.15), ncol=min(len(exp_data['model'].unique()), 4))
        ...
        plt.xlabel('Number Precision (size)', fontsize=12)
        plt.ylabel('PCA Component 1 Explained Variance (Mean ± Std)', fontsize=12)
        plt.legend(loc='lower center', bbox_to_anchor=(0.5, -0.15), ncol=min(len(exp_data['model'].unique()), 4))
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.subplots_adjust(bottom=0.2)
        ...
        plt.xlabel('Number Precision (size)', fontsize=12)
        plt.ylabel('PCA R² (Mean ± Std)', fontsize=12)
        ...
        plt.xlabel('Linear Model R² (Mean ± Std)', fontsize=12)
        plt.ylabel('PCA R² (Mean ± Std)', fontsize=12)
...
def create_multi_axis_plots(data_subset, plot_type, save_dir, title_suffix=""):
    """Create multi-axis plots showing all three datasets together with k-fold error bars"""
    
    if len(data_subset) == 0:
        return
    
    # Get global bounds for consistent y-axis scaling using means
    ...
    # Plot 1: Linear R² vs Number Precision (all experiments) with error bars
    for idx, experiment in enumerate(experiment_names):
        ...
        ax.set_xlabel('Number Precision (size)', fontsize=12)
        if idx == 0:
            ax.set_ylabel('Linear R² (Mean ± Std)', fontsize=12)
    ...
    fig.legend(handles, labels, loc='lower center', bbox_to_anchor=(0.5, -0.1), ncol=min(len(labels), 6))
    ...
    # Plot 2: Explained Variance Ratio vs Number Precision (all experiments) with error bars
    fig, axes = plt.subplots(1, 3, figsize=(18, 6))
    ...
    for idx, experiment in enumerate(experiment_names):
        ...
        ax.set_xlabel('Number Precision (size)', fontsize=12)
        if idx == 0:
            ax.set_ylabel('PCA Component 1 Explained Variance (Mean ± Std)', fontsize=12)
        ax.set_title(experiment.replace('_', ' ').title(), fontsize=12, fontweight='bold')
        ax.set_ylim(pca_var_lower - 0.05, pca_var_upper + 0.05)
        ax.grid(True, alpha=0.3)
    ...
    # Plot 3: PCA R² vs Number Precision (all experiments) with error bars
    fig, axes = plt.subplots(1, 3, figsize=(18, 6))
    ...
    for idx, experiment in enumerate(experiment_names):
        ...
        ax.set_xlabel('Number Precision (size)', fontsize=12)
        if idx == 0:
            ax.set_ylabel('PCA R² (Mean ± Std)', fontsize=12)
        ax.set_title(experiment.replace('_', ' ').title(), fontsize=12, fontweight='bold')
    ...
    # Plot 4: PCA R² vs Linear R² (all experiments) with error bars
    fig, axes = plt.subplots(1, 3, figsize=(18, 6))
    ...
    for idx, experiment in enumerate(experiment_names):
        ...
        ax.set_xlabel('Linear Model R² (Mean ± Std)', fontsize=12)
        if idx == 0:
            ax.set_ylabel('PCA R² (Mean ± Std)', fontsize=12)
        ax.set_title(experiment.replace('_', ' ').title(), fontsize=12, fontweight='bold')
...
print("\n" + "="*60)
print("CREATING MULTI-AXIS PLOTS FOR GENERAL MODELS WITH K-FOLD ERROR BARS")
print("="*60)
...
if len(specialist_models) > 0:
    print("\n" + "="*60)
    print("CREATING MULTI-AXIS PLOTS FOR SPECIALIST MODELS WITH K-FOLD ERROR BARS")
    print("="*60)
...
print("\n" + "="*60)
print("CREATING MULTI-AXIS PLOTS BY PROVIDER (GENERAL MODELS ONLY) WITH K-FOLD ERROR BARS")
print("="*60)
...
if len(specialist_models) > 0:
    for provider in specialist_models['provider'].unique():
        ...
        os.makedirs(provider_dir, exist_ok=True)  # Create directory first
...
print("\n" + "="*60)
print("CREATING INDIVIDUAL PLOTS FOR GENERAL MODELS WITH K-FOLD ERROR BARS")
print("="*60)
create_plots(general_models, 'general', 'final_plots/general_models')

# Create plots for specialist models
print("\n" + "="*60)
print("CREATING INDIVIDUAL PLOTS FOR SPECIALIST MODELS WITH K-FOLD ERROR BARS")
print("="*60)
create_plots(specialist_models, 'specialist', 'final_plots/specialist_models')

# Create plots by provider (general models only)
print("\n" + "="*60)
print("CREATING INDIVIDUAL PLOTS BY PROVIDER (GENERAL MODELS ONLY) WITH K-FOLD ERROR BARS")
print("="*60)
...
if len(specialist_models) > 0:
    print("\n" + "="*60)
    print("CREATING INDIVIDUAL PLOTS BY PROVIDER (SPECIALIST MODELS) WITH K-FOLD ERROR BARS")
    print("="*60)
...
def create_latex_tables():
    """Generate LaTeX tables using pandas to_latex method - updated for k-fold format"""
    
    # Create tables directory
    os.makedirs('final_plots/tables', exist_ok=True)
    
    print("\nGenerating LaTeX tables using pandas (K-fold format)...")
    ...
    # Table 2: Performance Summary by Experiment and Provider (using k-fold means and stds)
    for experiment in experiments:
        ...
        for provider in sorted(exp_data['provider'].unique()):
            ...
            stats_list.append({
                ...
                'PCA Var Comp 1 (μ±σ)': f"{provider_data['pca_explained_var_mean'].mean():.3f}±{provider_data['pca_explained_var_mean'].std():.3f}"
            })
        ...
        with open(f'final_plots/tables/performance_summary_{experiment}.txt', 'w') as f:
            f.write(latex_table)
        print(f"✓ Generated performance_summary_{experiment}.txt")
    ...
    for experiment in experiments:
        ...
        for size in sorted(exp_data['size'].unique()):
            ...
            # Best linear R²
            best_linear = size_data.loc[size_data['linear_r2_mean'].idxmax()]
            # Best PCA R²
            best_pca = size_data.loc[size_data['pca_r2_mean'].idxmax()]
            
            best_models.append({
                'Size': int(size),
                'Best Linear Model': best_linear['model'],
                'Linear R²': f"{best_linear['linear_r2_mean']:.4f}",
                'Best PCA Model': best_pca['model'],
                'PCA R²': f"{best_pca['pca_r2_mean']:.4f}"
            })
        
        if best_models:  # Only create table if we have data
            ...
            with open(f'final_plots/tables/best_performance_{experiment}.txt', 'w') as f:
                f.write(latex_table)
            print(f"✓ Generated best_performance_{experiment}.txt")
    ...
    for experiment in experiments:
        ...
        for model in sorted(exp_data['model'].unique()):
            ...
            if len(model_data) > 2:  # Need at least 3 points for meaningful correlation
                corr_size_linear = model_data['size'].corr(model_data['linear_r2_mean'])
                corr_size_pca = model_data['size'].corr(model_data['pca_r2_mean'])
                corr_size_var = model_data['size'].corr(model_data['pca_explained_var_mean'])
    ...
    if correlation_list:
        ...
        with open('final_plots/tables/model_correlations.txt', 'w') as f:
            f.write(latex_table)
        print("✓ Generated model_correlations.txt")
    ...
    for experiment in experiments:
        ...
        corr_linear_pca = exp_data['linear_r2_mean'].corr(exp_data['pca_r2_mean'])
        corr_size_linear = exp_data['size'].corr(exp_data['linear_r2_mean'])
        corr_size_pca = exp_data['size'].corr(exp_data['pca_r2_mean'])
        corr_size_var = exp_data['size'].corr(exp_data['pca_explained_var_mean'])
        
        overall_correlation_data.append({
            'Experiment': experiment.replace('_', ' ').title(),
            'Linear vs PCA R²': f"{corr_linear_pca:.3f}" if not pd.isna(corr_linear_pca) else 'N/A',
            'Size vs Linear R²': f"{corr_size_linear:.3f}" if not pd.isna(corr_size_linear) else 'N/A',
            'Size vs PCA R²': f"{corr_size_pca:.3f}" if not pd.isna(corr_size_pca) else 'N/A',
            'Size vs PCA Var': f"{corr_size_var:.3f}" if not pd.isna(corr_size_var) else 'N/A'
        })
    
    if overall_correlation_data:
        ...
        with open('final_plots/tables/overall_correlations.txt', 'w') as f:
            f.write(latex_table)
        print("✓ Generated overall_correlations.txt")
    ...
    if len(specialist_models) > 0:
        ...
        for experiment in experiments:
            ...
            if len(exp_general) > 0 and len(exp_specialist) > 0:
                comparison_data.append({
                    ...
                    'General Linear R²': f"{exp_general['linear_r2_mean'].mean():.3f}±{exp_general['linear_r2_mean'].std():.3f}",
                    'General PCA R²': f"{exp_general['pca_r2_mean'].mean():.3f}±{exp_general['pca_r2_mean'].std():.3f}",
                    'Specialist Models': len(exp_specialist['model'].unique()),
                    'Specialist Linear R²': f"{exp_specialist['linear_r2_mean'].mean():.3f}±{exp_specialist['linear_r2_mean'].std():.3f}",
                    'Specialist PCA R²': f"{exp_specialist['pca_r2_mean'].mean():.3f}±{exp_specialist['pca_r2_mean'].std():.3f}"
                })
        ...
        if comparison_data:
            ...
            with open('final_plots/tables/general_vs_specialist.txt', 'w') as f:
                f.write(latex_table)
            print("✓ Generated general_vs_specialist.txt")
    ...
    for provider in sorted(df['provider'].unique()):
        ...
        provider_summary.append({
            ...
            'Avg Linear R²': f"{provider_data['linear_r2_mean'].mean():.3f}",
            'Avg PCA R²': f"{provider_data['pca_r2_mean'].mean():.3f}",
            'Avg PCA Var': f"{provider_data['pca_explained_var_mean'].mean():.3f}"
        })
    ...
    if provider_summary:
        ...
        with open('final_plots/tables/provider_summary.txt', 'w') as f:
            f.write(latex_table)
        print("✓ Generated provider_summary.txt")
    
    # Table 8: Simplified Score Ranges (VALUE ± DEVIATION format)
    simplified_ranges = []
    ...
    for experiment in sorted(df['experiment'].unique()):
        ...
        for model in sorted(exp_data['model'].unique()):
            ...
            provider = model_data['provider'].iloc[0]
            model_type = 'Specialist' if model_data['is_specialist'].iloc[0] else 'General'
            
            # Calculate min and max for means, then format with std at those points
            linear_r2_min_idx = model_data['linear_r2_mean'].idxmin()
            linear_r2_max_idx = model_data['linear_r2_mean'].idxmax()
            linear_r2_min_val = model_data.loc[linear_r2_min_idx, 'linear_r2_mean']
            linear_r2_min_std = model_data.loc[linear_r2_min_idx, 'linear_r2_std']
            linear_r2_max_val = model_data.loc[linear_r2_max_idx, 'linear_r2_mean']
            linear_r2_max_std = model_data.loc[linear_r2_max_idx, 'linear_r2_std']
            
            pca_r2_min_idx = model_data['pca_r2_mean'].idxmin()
            pca_r2_max_idx = model_data['pca_r2_mean'].idxmax()
            pca_r2_min_val = model_data.loc[pca_r2_min_idx, 'pca_r2_mean']
            pca_r2_min_std = model_data.loc[pca_r2_min_idx, 'pca_r2_std']
            pca_r2_max_val = model_data.loc[pca_r2_max_idx, 'pca_r2_mean']
            pca_r2_max_std = model_data.loc[pca_r2_max_idx, 'pca_r2_std']
            
            pca_var_min_idx = model_data['pca_explained_var_mean'].idxmin()
            pca_var_max_idx = model_data['pca_explained_var_mean'].idxmax()
            pca_var_min_val = model_data.loc[pca_var_min_idx, 'pca_explained_var_mean']
            pca_var_min_std = model_data.loc[pca_var_min_idx, 'pca_explained_var_std']
            pca_var_max_val = model_data.loc[pca_var_max_idx, 'pca_explained_var_mean']
            pca_var_max_std = model_data.loc[pca_var_max_idx, 'pca_explained_var_std']
            
            simplified_ranges.append({
                'Dataset': experiment.replace('_', ' ').title(),
                'Model': model,
                'Provider': provider,
                'Type': model_type,
                'Linear R² Min': f"{linear_r2_min_val:.4f} $\\pm$ {linear_r2_min_std:.4f}",
                'Linear R² Max': f"{linear_r2_max_val:.4f} $\\pm$ {linear_r2_max_std:.4f}",
                'PCA R² Min': f"{pca_r2_min_val:.4f} $\\pm$ {pca_r2_min_std:.4f}",
                'PCA R² Max': f"{pca_r2_max_val:.4f} $\\pm$ {pca_r2_max_std:.4f}",
                'PCA Var Min': f"{pca_var_min_val:.4f} $\\pm$ {pca_var_min_std:.4f}",
                'PCA Var Max': f"{pca_var_max_val:.4f} $\\pm$ {pca_var_max_std:.4f}"
            })
    
    if simplified_ranges:
        simplified_ranges_df = pd.DataFrame(simplified_ranges)
        
        latex_table = simplified_ranges_df.to_latex(
            index=False,
            caption='Simplified Score Ranges by Model and Dataset (VALUE $\\pm$ DEVIATION)',
            label='tab:simplified_ranges',
            position='htbp',
            column_format='llcccccccc',
            escape=False
        )
        
        with open('final_plots/tables/simplified_score_ranges.txt', 'w') as f:
            f.write(latex_table)
        print("✓ Generated simplified_score_ranges.txt")
    
    print(f"\nGenerated LaTeX tables for {len(experiments)} experiments with k-fold statistics")
...
# Summary statistics
print("\n" + "="*60)
print("K-FOLD SUMMARY STATISTICS")
print("="*60)

print("\nGENERAL MODELS (K-FOLD):")
for experiment in general_models['experiment'].unique():
    ...
    for provider in exp_data['provider'].unique():
        ...
        print(f"    K-folds: {provider_data['k_folds'].iloc[0] if len(provider_data) > 0 else 'N/A'}")
        print(f"    Model type: {provider_data['model_type'].iloc[0] if len(provider_data) > 0 else 'N/A'}")
        print(f"    Linear R² range (mean): {provider_data['linear_r2_mean'].min():.4f} - {provider_data['linear_r2_mean'].max():.4f}")
        print(f"    PCA R² range (mean): {provider_data['pca_r2_mean'].min():.4f} - {provider_data['pca_r2_mean'].max():.4f}")
        print(f"    Avg Linear R² std: {provider_data['linear_r2_std'].mean():.4f}")
        print(f"    Avg PCA R² std: {provider_data['pca_r2_std'].mean():.4f}")

if len(specialist_models) > 0:
    print("\nSPECIALIST MODELS (K-FOLD):")
...
print(f"\n" + "="*60)
print("K-FOLD LATEX TABLES GENERATED (.txt files using pandas to_latex):")
print("final_plots/tables/")
print("├── model_overview.txt")
print("├── performance_summary_[experiment].txt (with k-fold means & stds)")
print("└── ... (other tables updated for k-fold format)")
print("="*60)

print(f"\n" + "="*60)
print("DIRECTORY STRUCTURE CREATED (K-FOLD WITH ERROR BARS):")
print("final_plots/")
print("├── general_models/ (with k-fold error bars)")
print("├── specialist_models/ (with k-fold error bars)")
print("├── by_provider/ (with k-fold error bars)")
print("│   ├── openai/")
print("│   ├── google/")
print("│   └── voyage/")
print("└── tables/ (k-fold statistics)")
print("="*60)

print(f"\n" + "="*60)
print("K-FOLD VISUALIZATION SUMMARY:")
print(f"- All plots now include error bars showing ± standard deviation across {df['k_folds'].iloc[0] if len(df) > 0 else 'N/A'} folds")
print(f"- Using k-fold mean values for performance metrics")
print(f"- Error bars show cross-validation uncertainty")
print(f"- Model type: {df['model_type'].iloc[0] if len(df) > 0 else 'N/A'}")
print("- Updated column names: linear_r2_mean/std, pca_r2_mean/std, pca_explained_var_mean/std")
print("="*60)